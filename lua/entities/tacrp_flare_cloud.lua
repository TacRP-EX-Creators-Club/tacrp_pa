...
function ENT:Think()
    if CLIENT then
        ...
        if self.Ticks % math.ceil(6 + d * 10)  == 0 then
            local fire = emitter:Add("effects/spark", self:GetPos() + Vector(math.Rand(-4, 342), math.Rand(-4, 324), 0))
            fire:SetVelocity( VectorRand() * 300 + self:GetAngles():Up() * 500 )
        ...
        if self.NextFlareTime <= CurTime() then
            self.NextFlareTime = CurTime() + math.Rand(0.3, 0.8)
